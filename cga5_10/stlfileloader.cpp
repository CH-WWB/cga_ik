...
STLFileLoader::STLFileLoader(QString filename, float ratio, QObject *parent):QObject(parent)
{
  mRatio = ratio;
  model.clear();
  m_filename = filename;
...
//    qDebug() <<arr;
  ...
  m_isFinished = true;
  emit finished();//发送 finished 信号，通知其他对象文件读取已完成
...
void STLFileLoader::load()
{
    loadStl(m_filename);
}

bool STLFileLoader::isFinished() const
{
    return m_isFinished;
}
